...
export async function loadImportHistory(){
 ...
 try{const r=await fetch('/api/import-history');if(!r.ok)throw new Error();const remote=await r.json();return Array.isArray(remote)?[...remote,...local].sort((a,b)=>new Date(b.importedAt).getTime()-new Date(a.importedAt).getTime()).slice(0,250):local}catch{return local}
}
export async function deleteImportHistory(ids:Array<string|number>){
 ...
 const r=await fetch('/api/import-history',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids:remoteIds})});if(!r.ok)throw new Error('History delete failed');return r.json();
}
