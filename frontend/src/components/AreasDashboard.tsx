...
export function AreasDashboard({orders}:{orders:Order[]}){
 ...
 return <div className="space-y-4">
  ...
  <section className="bg-white rounded-2xl border overflow-hidden" aria-labelledby="priority-areas-heading">
   <div className="p-4 border-b bg-slate-50"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><h3 id="priority-areas-heading" className="font-bold text-slate-900">Priority Areas for Dispatch</h3><p className="text-xs text-slate-500 mt-1">Every order from Today’s Orders, RTG and Unfinished is listed with its area code.</p></div><div className="flex gap-2"><span className="rounded-full bg-violet-100 text-violet-800 px-3 py-1 text-xs font-bold">{priorityOrders.length} orders</span><span className="rounded-full bg-slate-200 text-slate-700 px-3 py-1 text-xs font-bold">{priorityAreaCount} areas</span></div></div></div>
   <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 p-3">{priorityOrders.map(({order,area,today,rtg,unfinished})=>{const number=order.skNumber||order.voucherNumber||order.partyOrderNumber||'No Order Number';const color=rtg?'border-cyan-300 bg-cyan-50':unfinished?'border-amber-300 bg-amber-50':'border-indigo-300 bg-indigo-50';return <article key={order.id} className={`rounded-xl border p-3 ${color}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Area Code</div><b className="text-base text-slate-950 break-words">{area}</b></div><div className="flex flex-wrap justify-end gap-1">{today&&<span className="rounded bg-indigo-600 text-white px-2 py-0.5 text-[10px] font-bold">TODAY</span>}{rtg&&<span className="rounded bg-cyan-700 text-white px-2 py-0.5 text-[10px] font-bold">RTG</span>}{unfinished&&<span className="rounded bg-amber-500 text-white px-2 py-0.5 text-[10px] font-bold">UNFINISHED</span>}</div></div><div className="mt-3 border-t border-slate-200 pt-2"><div className="font-semibold text-sm text-slate-900 truncate" title={order.companyName}>{order.companyName}</div><div className="mt-1 flex items-center justify-between gap-2 text-xs"><span className="font-mono font-bold text-blue-700 truncate" title={number}>{number}</span><span className="text-slate-600 whitespace-nowrap">Pending <b>{Math.max(0,order.pending).toLocaleString('en-IN')}</b></span></div></div></article>})}{!priorityOrders.length&&<div className="sm:col-span-2 lg:col-span-3 xl:col-span-4 p-6 text-center text-sm text-slate-500">No Today, RTG or Unfinished orders right now.</div>}</div>
  </section>
